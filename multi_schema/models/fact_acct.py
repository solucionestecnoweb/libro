...
class AccountMove(models.Model):
    ...
    def default_currency(self):
        ...
        currency = journal.company_id.currency_id.id
        return currency
    ...
    def block_rate(self):
        for record in self.invoice_line_ids:
            if record.product_id or not record.product_id:
                self.execute_line = True

    @api.onchange('currency_from_id')
    def _compute_rate(self):
        if self.currency_from_id:
            rate = utils.get_invoice_rate(self)
...
class Currency(models.Model):
    ...
    def open_currency_rate(self):
        results = self.env['multi.currency.rate'].read_group([('currency_id', 'in', self.ids)], ['currency_id'],
                                                             ['currency_id'])
        dic = {}
        for x in results: dic[x['currency_id'][0]] = x['currency_id_count']
        for record in self: record['currency_rates'] = dic.get(record.id, 0)
