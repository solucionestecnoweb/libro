...
class BsoftContratoReport2(models.TransientModel):
    ...
    def action_generate_txt(self):
        ...
        ret_cursor = self.env['account.move'].search([('invoice_date','>=',self.date_from),('invoice_date','<=',self.date_to),('type','in',('in_invoice','in_refund','in_receipt')),('state','=','posted'),])
                    ...
                    if ret.vat_ret_id.state=="posted":
                    	...
                    	for rec in rec_cursor:
                    		...
                    		if rec.tax_id.aliquot!="exempt":
		                        ...
		                        fecha = rec.invoice_id.invoice_date
		                        fecha = str(fecha)
		                        file.write(fecha + "\t")#3
